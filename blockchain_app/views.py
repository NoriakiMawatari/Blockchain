...
def fetch_posts():
    ...
    print(f"ChainADDRESS {get_chain_address}")
    response = requests.get(get_chain_address)
    print(f"Response for the chain address gotten {response}")
    if response.status_code == 200:
        ...
        for block in chain["chain"]:
            for tx in block["transactions"]:
                ...
                content.append(tx)
        global posts
        posts = sorted(content, key=lambda key: key['timestamp'], reverse=True)
